...
import AdminKelurahanForm from "../admin/AdminKelurahanForm";
import {
  adminKelurahanDelete,
  adminKelurahanIndex,
  adminKelurahanStore,
  adminKelurahanUpdate,
} from "../../api/admin/AdminKelurahanApi";
import {
  Row,
  Col,
  Button,
...
export default function AdminKelurahan() {
  ...
  const fetchDatas = useCallback(async (params) => {
    try {
      ...
      await adminKelurahanIndex(params).then((response) => {
      ...
      case "delete":
        try {
          if (
            ...
            await adminKelurahanDelete({ id: payload.id }).then((_) => {
              setFilter({ ...filter });
  ...
  async function tableModalOnOk() {
    try {
      ...
      switch (params["mode"]) {
        case "add":
          await adminKelurahanStore(params).then((_) => {
            setModal({ ...modal, visible: false });
        ...
        case "edit":
          await adminKelurahanUpdate(params).then((_) => {
            setModal({ ...modal, visible: false });
  ...
  return (
    <div>
      ...
      <Title level={4}>Daftar Kelurahan</Title>
