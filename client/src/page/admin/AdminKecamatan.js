...
import AdminKecamatanForm from "../admin/AdminKecamatanForm";
import {
  adminKecamatanDelete,
  adminKecamatanIndex,
  adminKecamatanStore,
  adminKecamatanUpdate,
} from "../../api/admin/AdminKecamatanApi";
import {
  Row,
  Col,
  Button,
...
export default function AdminKecamatan() {
  ...
  const fetchDatas = useCallback(async (params) => {
    try {
      ...
      await adminKecamatanIndex(params).then((response) => {
      ...
      case "delete":
        try {
          if (
            ...
            await adminKecamatanDelete({ id: payload.id }).then((_) => {
              setFilter({ ...filter });
  ...
  async function tableModalOnOk() {
    try {
      ...
      switch (params["mode"]) {
        case "add":
          await adminKecamatanStore(params).then((_) => {
            setModal({ ...modal, visible: false });
        ...
        case "edit":
          await adminKecamatanUpdate(params).then((_) => {
            setModal({ ...modal, visible: false });
  ...
  return (
    <div>
      ...
      <Title level={4}>Daftar Kecamatan</Title>
