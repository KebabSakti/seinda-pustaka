...
import {
  adminKabupatenDelete,
  adminKabupatenIndex,
  adminKabupatenStore,
  adminKabupatenUpdate,
} from "../../api/admin/AdminKabupatenApi";
...
import AdminKabupatenForm from "./AdminKabupatenForm";
...
export default function AdminKabupaten() {
  ...
  const fetchDatas = useCallback(async (params) => {
    try {
      ...
      await adminKabupatenIndex(params).then((response) => {
      ...
      case "delete":
        try {
          if (
            ...
            await adminKabupatenDelete({ id: payload.id }).then((_) => {
              setFilter({ ...filter });
  ...
  async function tableModalOnOk() {
    try {
      ...
      switch (params["mode"]) {
        case "add":
          await adminKabupatenStore(params).then((_) => {
            setModal({ ...modal, visible: false });
        ...
        case "edit":
          await adminKabupatenUpdate(params).then((_) => {
            setModal({ ...modal, visible: false });
  ...
  return (
    <div>
      ...
      <Title level={4}>Daftar Kabupaten</Title>
